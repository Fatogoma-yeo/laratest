...
<div class="py-2 flex justify-between">
    <label for="title" class="font-bold my-2 text-primary text-white">
        <h1 class="text-2xl font-bold">Modifier la Dépense</h1>
        <h3>Modifier la dépense et enregistrez-la.</h3>
    </label>
    <div for="title" class="text-sm my-2 text-primary">
        <a href="{{ route('expense_categories.index') }}" class="rounded-full border ns-inset-button error hover:bg-gray-200 hover:text-gray-900 text-white flex px-1 py-1">
...
<div class="">
    <form method="POST" action="{{ route('expense_categories.update', $expenseCategory->id) }}" id="editForm">
        @method('PUT')
        @csrf
        ...
        <div class="py-4">
            ...
            <div class="flex justify-between rounded-md border-2 bg-indigo-600 border-indigo-600">
                <x-input id="name" class="block w-full" type="text" name="name" :value="$expenseCategory->name" required autofocus />
                <x-button>
                    {{ __('Sauvegarder') }}
                </x-button>
            </div>
            ...
            <div class="grid grid-cols-1 pb-8">
                ...
                <div class="col-span-1 mt-4">
                    ...
                    <select name="operation" id="operation" class="mt-1 block w-full py-2 px-3 border-gray-300 bg-gray-100 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                        <option value="{{$expenseCategory->operation}}">@if($expenseCategory->operation == "debit") Débit @else Crédit @endif</option>
