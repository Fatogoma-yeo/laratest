...
@else
    @foreach($productsDetails as $product)
        <div class="products_item flex" id="content">
                ...
                <div class="flex justify-between product-details mb-1">
                    ...
                    <div class="-mx-1 flex product-options">
                        ...
                        <div class="px-1 border-dashed py-1 border-b-2 @if($product['is_gross'] == 1) border-green-600 @else border-blue-400 @endif">
                            <a onclick="gross_purchase('{{ $product['product_id'] }}')" class="cursor-pointer outline-none border-dashed py-1 border-b-2 text-sm @if($product['is_gross'] == 1) text-green-600 hover:text-green-700 @else hover:text-blue-400 @endif">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                ...
                <div class="flex justify-between product-controls">
                    <div class="-mx-1 flex flex-wrap">
                        <div class="px-1 w-1/2 md:w-auto mb-1">
                            ...
                            <a class="outline-none border-dashed py-1 border-b-2 text-sm @if($product['is_gross'] == 1) border-green-600 text-green-600 @else border-blue-400 @endif">
                                {{ __( 'Price' ) }} :
                                <span id="net_purchase_price">
                                  @if($product['is_gross'] == 1)
                                    @currency($product["gross_purchase_price"])
                                  @else
                                    @currency($product["net_purchase_price"])
                                  @endif
                                </span>
                            </a>
                        </div>
                        <div class="px-1 w-1/2 md:w-auto mb-1">
                            <a onclick="openDiscountPopup('{{ $product['product_id'] }}')" class="cursor-pointer outline-none border-dashed py-1 border-b-2 border-blue-400 text-sm">{{ __( 'Discount' ) }} :  <span class="hidden" id="discountValue">{{ $product['discount'] }}</span>@currency( $product['discount'] )</a>
                        </div>
                        ...
                        <div class="px-1 w-1/2 md:w-auto mb-1 lg:hidden">
                            ...
                            <span class="cursor-pointer outline-none border-dashed py-1 border-b-2 border-blue-400 text-sm">
                              {{ __( 'Total' ) }}:
                              <span class="hidden" id="total_purchase_price">
                                @if($product['is_gross'] == 1)
                                ...
                                @if($product['is_gross'] == 1)
                                  @currency( $product["quantity"] * $product["gross_purchase_price"] )
                                @else
                                  @currency( $product["quantity"] * $product["net_purchase_price"] )
                                @endif
                            </span>
                        </div>
                    </div>
            ...
            <span class="hidden" id="pending_pos_discount">{{ $product['pos_discount'] }}</span>
